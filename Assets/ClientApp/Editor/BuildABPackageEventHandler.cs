...
public sealed class BuildABPackageEventHandler : IBuildEventHandler
{
    ...
    public void PostProcessBuild(BuildTarget buildTarget, string workingPath, string outputPackagePath, string outputFullPath, string outputPackedPath)
    {
        ...
        if (
            Application.platform == RuntimePlatform.Android &&  buildTarget == BuildTarget.Android ||
            Application.platform == RuntimePlatform.IPhonePlayer && buildTarget == BuildTarget.iOS ||
            (Application.platform == RuntimePlatform.WindowsEditor || Application.platform  == RuntimePlatform.WindowsPlayer) && (buildTarget == BuildTarget.StandaloneWindows || buildTarget == BuildTarget.StandaloneWindows64)
        )
        {
